...
class CLS_REGISTER{
	...
	public function getList($where='',$limit=''){
		$sql="SELECT * FROM `tbl_register` where 1=1 ".$where.$limit;
	...
	function listTable($strwhere="",$page=1){
		$sql="SELECT * FROM tbl_register ".$strwhere ;
    ...
    public function getCount($where=""){
        $sql="SELECT COUNT(*) as 'number' FROM `tbl_register` WHERE isactive=1 " . $where;
        $objdata = new CLS_MYSQL();
        $objdata->Query($sql);
	...
	function getChildID($parid) {
		$sql = "SELECT id FROM tbl_register WHERE par_id IN ('$parid')"; 
		$objdata = new CLS_MYSQL; 
	...
	public function updateView($id){
        if(!isset($_SESSION['VIEW-ALBUM-'.$id])){
            $sql="UPDATE `tbl_register` SET `visited`=`visited`+1 WHERE `id` ='$id'";
