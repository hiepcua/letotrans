...
$sql="SELECT COUNT(*) AS count FROM tbl_package WHERE 1=1 ".$strwhere;
$objmysql->Query($sql);
...
<div id="path">
    <ol class="breadcrumb">
        ...
        <li class="active">Danh sách gói dịch vụ</li>
    </ol>
</div>
...
<div class="table-responsive">
  <table class="table table-bordered">
     <tr class="header">
        ...
        <th align="center">Gói cơ bản</th>
        <th align="center">Gói PRO</th>
    ...
    $star = ($cur_page - 1) * 20;
    $leng = 20;
    $sql = "SELECT * FROM tbl_package WHERE 1=1 ".$strwhere." ORDER BY `order` ASC LIMIT ".$star.", ".$leng;
    ...
    while($rows = $objmysql->Fetch_Assoc()){
        ...
        echo "<td>
        <input class='ajax-price' data-id='".$ids."' onchange=\"ajax_update_price(this)\" type='text' name='txt_price_basic' value='".$rows['price_basic']."'>
        </td>";

        echo "<td>
        <input class='ajax-price' data-id='".$ids."' onchange=\"ajax_update_price(this)\" type='text' name='txt_price_pro' value='".$rows['price_pro']."'>
        </td>";

        echo "<td>
        <input class='ajax-price' data-id='".$ids."' onchange=\"ajax_update_price(this)\" type='text' name='txt_price_vip' value='".$rows['price_vip']."'>
        </td>";
