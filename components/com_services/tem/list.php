			...
			<div class="list-service">
				...
				while ($r_service = $objmysql->Fetch_Assoc()) {
					...
					if($service_type !== []){
						...
						$sql1 = "SELECT * FROM tbl_service_type WHERE isactive = 1 AND id IN(".$service_type.")";
						$objdata->Query($sql1);
						...
						while ($r1 = $objdata->Fetch_Assoc()) {
							if($i == 1){
								$link_order = ROOTHOST.'order?service='.$r_service['id'].'&service_type='.$r1['id'].'&package=1';
							}
						...
						$shtml2.= '<div class="header-title">BẢNG GIÁ DỊCH THEO TỪNG TỪ</div>';
					...
					if($r_service['name'] == "Dịch bản địa hóa"){
						$tmp = 'LOẠI SẢN PHẨM';
					}else{
						$tmp = 'BẢNG GIÁ DỊCH THEO TỪNG TỪ';
					}
